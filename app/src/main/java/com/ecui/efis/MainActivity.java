...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    protected ImageView ivHSI, ivBankPointer, ivASI, ivAsTrendUp, ivAsTrendDown;
    protected TextView tvCurrentAirspeed, tvHeading, tvCurrentAlt, tvInfo;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ivHSI.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        isTvInfoShow=!isTvInfoShow;
                    }
                });
            }
        });
            ...
            public void run() {
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(isTvInfoShow){
                            tvInfo.setText(String.format("SPD:%.3fMPS(%.3fKPH)\nACC:%.3fm/s/s(%.3fkm/h/s)\nALT:%" +
                                            ".3fm\nAOA:%.1f%c\nBANK ANGLE:%.1f%c",
                                    speedMps,
                                    3.6*speedMps,accelerometerReading[1],3.6*accelerometerReading[1],altMeters,
                                    Math.toDegrees(Math.abs(orientationAngles[1])),orientationAngles[1]<=0?'U':'D',
                                    Math.toDegrees(Math.abs(orientationAngles[2])),orientationAngles[2]<=0?'L':'R'));
                        }
                        else{
                            tvInfo.setText("");
                        }
    ...
    public void updateHSIDisplay() {
        float bankAngleDegree = (float) Math.toDegrees(orientationAngles[1]);
        float unitsPerBankAngleDegree = 12.0f;
        // It seems that the translation and rotation for matrices have different reference points yet identical units.
        HSIMatrix.setTranslate(-935, -935 - bankAngleDegree * unitsPerBankAngleDegree);
        HSIMatrix.preRotate((float) Math.toDegrees(-orientationAngles[2]), 1199, 1199 + bankAngleDegree * unitsPerBankAngleDegree);
        ivHSI.setImageMatrix(HSIMatrix);

        ivBankPointer.setRotation((float) Math.toDegrees(-orientationAngles[2]));
    }

    public void updateASTrendVector() {
    ...
    public void updateHeadingDisplay() {
